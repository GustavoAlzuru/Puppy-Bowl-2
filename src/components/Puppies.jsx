...
const ListOfPuppies = ({ allPuppies, newPuppy, setFilteredPuppies, setNewPuppy }) => {
                ...
                <li className='card-puppies' key={puppies?.id}>
                    ...
                    <section className='puppies-info'>
                        ...
                        {newPuppy?.some(puppy => puppy.id === puppies.id) && (
                            <Delete pupId={puppies?.id} allPuppies={allPuppies} setFilteredPuppies={setFilteredPuppies} setNewPuppy={setNewPuppy} newPuppy={newPuppy}/>
                        )}
                    </section>
                </li>
            ))}
...
const Puppies = ({ allPuppies, setFilteredPuppies, newPuppy, setNewPuppy }) => {
    const hasPuppies = allPuppies?.length > 0
    return (
        hasPuppies
            ? <ListOfPuppies allPuppies={allPuppies} setFilteredPuppies={setFilteredPuppies} newPuppy={newPuppy} setNewPuppy={setNewPuppy}/>
            : <NoPuppiesResult />
    );
}
